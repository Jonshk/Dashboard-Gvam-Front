// src/app/pages/dashboard/stock/stock.page.ts
...
export class StockPageComponent implements OnInit {
  ...
  get filteredItems(): StockItemAPI[] {
    if (!this.filterTerm) return this.items;
  ...
  save(): void {
    ...
    // Extraemos el ID y el resto del payload
    const id = v.id as number | null;
    const payload: StockItemCreateAPI = {
    ...
    // Si hay ID => actualización, si no => creación
    const obs = id
      ? this.stockSvc.updateItem(id, payload)
      : this.stockSvc.createItem(payload);
    ...
    obs.subscribe({
      ...
      error: err => console.error('Error guardando item', err)
    });
  }

  deleteItem(item: StockItemAPI): void {
    if (!confirm(`¿Eliminar "${item.product_name}"?`)) return;
    this.stockSvc.deleteItem(item.id)
